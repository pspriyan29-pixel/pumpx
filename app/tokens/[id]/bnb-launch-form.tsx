...
export default function BnbLaunchForm({ tokenId }: { tokenId: string }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setMsg(null)
    try {
      const res = await fetch('/api/launch/bnb', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tokenId, tokenAddress, baseAmount, quoteAmount })
      })
  ...
  return (
    <section className="rounded-lg border border-zinc-800 p-4 space-y-2">
      <div className="text-sm font-medium">Launch to DEX (BNB) — LP Params</div>
      <form onSubmit={submit} className="grid gap-2 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="block text-xs mb-1">Token Address</label>
        ...
        <div>
          <label className="block text-xs mb-1">Quote amount (BNB/USDT)</label>
